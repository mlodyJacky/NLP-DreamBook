...
def create_summary(interpretation, recommendation, predicted_emotion):
    template = """
        ...
        scale of 0 to 100. Summarize these three parameters in up to 500 characters in up to three sentences where each
          sentence is about a separate parameter. do not divide the answer into 3 sections just write everything in one
          paragraph, maximum 3 sentences, the first sentence refers to the interpretation, the second sentence to the
            recommendation and the third sentence to the user's feelings. 
        interpretation: {interpretation}
        recommendation: {recommendation}
        predicted_emotions: {predicted_emotion}
    ...
    result = chain.invoke({"interpretation": interpretation, "recommendation": recommendation, "predicted_emotion": predicted_emotion})
    return result
